...
class SimpleRNN:
    ...
    def generate(self, pi, word2idx):
        ...
        while n_lines < 16:
            P = self.predict_op(X)[-1] #keeping just the last word
            X += [P]
            if P > 1:
                word = idx2word[P]
                print (word, end=' ')
...
def train_poetry():
    ...
    rnn.save('RNN_30D_30M_epochs2000_relu-2.npz')

def generate_poetry():
    sentences, word2idx = get_robert_frost()
    rnn = SimpleRNN.load('RNN_30D_30M_epochs2000_relu.npz', T.nnet.relu)
