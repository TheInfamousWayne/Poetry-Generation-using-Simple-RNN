# -*- coding: utf-8 -*-
"""
Created on Thu Dec 21 20:19:38 2017
...
from utils import init_weight, get_robert_frost
...
class RRNN:
    ...
    def fit(self, X, learning_rate=10e-1, mu=0.99, reg=1.0, activation=T.tanh, epochs=500, show_fig=False):
        ...
        bh = np.zeros(M)
        h0 = np.zeros(M)
        #z = np.zeros(M)  #we'll make z(t) dependent on Xt and Ht-1
        Wxz = init_weight(D, M)
        Whz = init_weight(M, M)
        bz = np.zeros(M)
        Wo = init_weight(M, V)
        ...
        for i in range(epochs):
            ...
            for j in range(N):
                if np.random.random() < 0.1 :
                    ...
                    input_sequece = [0] + X[j][:-1]
                    output_sequence = X[j]
                n_total += len(output_sequence)
                c, p = self.train_op(input_sequece,output_sequence,learning_rate)
                cost += c
...
# END of SimpleRNN Class

def train_poetry():
    sentences, word2idx = get_robert_frost()
    rnn = RRNN(50, 50, len(word2idx))
    rnn.fit(sentences, learning_rate=10e-5, activation=T.nnet.relu, epochs=2000, show_fig=True)
    rnn.save('RRNN_50D_50M_epochs2000_relu.npz')

def generate_poetry():
    sentences, word2idx = get_robert_frost()
    rnn = RRNN.load('RRNN_50D_50M_epochs2000_relu.npz', T.nnet.relu)
